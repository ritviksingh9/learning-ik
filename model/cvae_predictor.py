...
class PredictCVAE(nn.Module):
    def __init__(self, config: Optional[dict] = None):
        super().__init__()
        # updated config
        self._config = PREDICT_CVAE_DEFAULT_CONFIG
        ...
        # predictor takes in desired pose
        self.predictor = nn.Sequential(
            nn.Linear(self._config["pose_dims"], self._config["hidden_dims"]),
            nn.ReLU(),
            nn.Linear(self._config["hidden_dims"], self._config["hidden_dims"]),
            nn.ReLU(),
            nn.Linear(self._config["hidden_dims"], self._config["hidden_dims"]),
            nn.ReLU(),
            nn.Linear(self._config["hidden_dims"], self._config["pose_dims"])
        )
    ...
    def forward(self, joint_config: torch.Tensor, desired_pose: torch.Tensor) -> Tuple[torch.Tensor]:
        ...
        # predictor
        prediction = self.predictor(desired_pose.view(-1, self._config["pose_dims"]))
        # sample mean
        ...
        # run through decoder
        return self.decoder(torch.cat((z, prediction), axis=1)), mean, log_variance
